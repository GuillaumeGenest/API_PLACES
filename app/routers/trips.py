...
async def get_trip_in_city(city_name: str, firstdate: date, lastdate: date, user: dict = Depends(get_current_user)):
    ...
    service = TripService()
    data = await service.generate_city_trip(city_name, str(firstdate), str(lastdate), user_id=user["sub"])
    if data:
...
async def get_roadtrip(region: str, firstdate: date, lastdate: date, user: dict = Depends(get_current_user)):
    logger.info(f"HTTP | GET /trips/roadtrip — region={region} from={firstdate} to={lastdate}")
    if firstdate > lastdate:
        logger.warning(f"HTTP | Dates invalides — from={firstdate} to={lastdate}")
        raise HTTPException(status_code=400, detail="La date de début doit être avant la date de fin")
    service = TripService()
    data = await service.generate_road_trip(region, str(firstdate), str(lastdate), user_id=user["sub"])
    if data:
