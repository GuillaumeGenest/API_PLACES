...
class TestGetImages(unittest.TestCase):
    ...
    def test_get_image_by_name_not_found(self):
        with patch('app.api.API_Photos.get_url_image_from_wikipedia', return_value=None):
    ...
    def test_get_image_by_place_and_address_success(self):
        with patch.object(images_module.places_service, 'get_place_id', return_value="test_place_id"), \
             patch.object(images_module.photos_service, 'get_image_by_place_id', return_value="http://example.com/photo.jpg"), \
             ...
             patch('app.routers.images.download_and_store', return_value="http://localhost:8000/images/storage/caches/test_place_id.jpg"):
    ...
    def test_get_image_by_place_and_address_place_not_found(self):
        with patch.object(images_module.places_service, 'get_place_id', return_value=None):
    ...
    def test_get_image_by_place_and_address_no_photo(self):
        with patch.object(images_module.places_service, 'get_place_id', return_value="test_place_id"), \
             patch.object(images_module.photos_service, 'get_image_by_place_id', return_value=None), \
             patch('app.routers.images.is_stored', return_value=False), \
             patch('app.routers.images.get_storage_url', return_value="http://localhost:8000/images/storage/caches/test_place_id.jpg"), \
             patch('app.routers.images.download_and_store', return_value=None):
            response = self.client.get("/images/place_and_address?place_name=Tour+Eiffel&address=Paris")
            self.assertEqual(response.status_code, 404)
            self.assertIn("IMAGE_NOT_FOUND", response.json()["error"])
    ...
    def test_get_image_by_id_success(self):
        with patch.object(images_module.photos_service, 'get_image_by_place_id', return_value="http://example.com/photo.jpg"), \
             patch('app.routers.images.is_stored', return_value=False), \
             patch('app.routers.images.get_storage_url', return_value="http://localhost:8000/images/storage/caches/test_place_id.jpg"), \
             patch('app.routers.images.download_and_store', return_value="http://localhost:8000/images/storage/caches/test_place_id.jpg"):
    ...
    def test_get_image_by_id_no_photo(self):
        with patch.object(images_module.photos_service, 'get_image_by_place_id', return_value=None), \
             patch('app.routers.images.is_stored', return_value=False), \
             patch('app.routers.images.get_storage_url', return_value="http://localhost:8000/images/storage/caches/test_place_id.jpg"), \
             patch('app.routers.images.download_and_store', return_value=None):
            response = self.client.get("/images/id?place_id=test_place_id")
